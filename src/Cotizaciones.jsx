    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:20 }}>
        <div style={{ background:'white', border:`1px solid ${COLORS.slate100}`, borderRadius:12, overflow:'hidden' }}>
          ...
          {cot.items.map(item => (
            <div key={item.id} style={{ display:'grid', gridTemplateColumns:'30px 1fr 100px 110px 140px 80px 40px', padding:'12px 20px', borderBottom:`1px solid ${COLORS.slate100}`, alignItems:'center', fontSize:12 }}>
              ...
              {cot.estado === 'Borrador' && <button onClick={() => delItem(item.id)} style={{ border:'none', background:'transparent', color:COLORS.red, cursor:'pointer' }}>{Icon('Trash')}</button>}
            </div>
          ))}
        ...
        <div>
          <div style={{ background:'white', border:`1px solid ${COLORS.slate100}`, borderRadius:12, padding:20, marginBottom:16 }}>
            <h3 style={{ fontSize:11, fontWeight:600, color:COLORS.slate500, marginBottom:14, textTransform:'uppercase', letterSpacing:'0.08em' }}>Información</h3>
