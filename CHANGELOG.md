...
- **v18.10.0** (22 jun) — **2 features (feedback del equipo)**. (1) **Cancelar sub-actividades en cascada (reversible)**: al poner una actividad en "Cancelada", si tiene sub-actividades pendientes la plataforma pregunta (modal) si cancelarlas también — *Cancelar N pendientes* / *Solo esta* / *No cancelar*. Cancela todos los descendientes (recursivo) **menos los Completados**. Es **reversible**: con Ctrl+Z, o regresando la actividad a cualquier estado ≠ Cancelada, las hijas auto-canceladas vuelven **exactamente a su estado anterior**. Mecanismo durable (sobrevive recarga/sesión): 2 columnas nuevas en `actividades` (`cancel_origen_id`, `cancel_estado_previo`) que marcan qué hija canceló qué origen y su estado previo; migración `v18.10.0_cancelar_cascada_reversible.sql` aplicada en prod. Lógica centralizada en `cambiarEstadoActividad` → cubre los 3 puntos de cambio de estado (select de la tabla, Gantt y panel lateral); nueva acción de undo `cancel-cascade`. **No toca dinero ni RLS** (columnas de estado, no de monto). (2) **PDF del Gantt: fechas inicio→fin**: cuando el toggle "Fechas" del Gantt está activo, el PDF exportado ahora muestra `inicio → fin` alineado a la derecha de la columna izquierda (columna ensanchada, nombre truncado contra ese espacio); con el toggle apagado el PDF queda idéntico al actual (`exportGantt.js` recibe `{ mostrarFechas }`). Build verificado.
- **v18.9.4** (19 jun) — **2 mejoras al Gantt (feedback del equipo)**. (1) **PDF: el nombre sale en TODAS las barras**. Antes la etiqueta junto a la barra se omitía si el nombre tenía >30 caracteres o no cabía a la derecha → unas salían y otras no; ahora se muestra siempre, a la derecha si hay espacio o a la izquierda de la barra si no, truncada al lado con más espacio (`exportGantt.js`). (2) **Gantt en pantalla: toggle "Fechas"** en la toolbar — muestra `inicio → fin` junto a cada actividad en el panel izquierdo (persistido en `gantt.fechas`, off por defecto). Build verificado.
- **v18.9.3** (18 jun) — **fix BD: estado 'Cancelada' permitido en actividades**. La constraint `actividades_estado_check` no lo incluía (error 23514 al guardar). Aplicado en prod vía `supabase db query --linked` (DROP/ADD constraint con los 6 estados) + migración versionada `v18.9.3_*`. Verificado con round-trip real.
- **v18.9.2** (17 jun) — **estado "Cancelada" para actividades**. Nueva opción en el selector de estado (tabla + panel), color gris apagado (inactivo). `estadoEfectivo` ya la preservaba (no la deriva a "Retrasada" aunque venza la fecha). `MisActividades` ya la tenía. No afecta columnas del Kanban (agrupa por urgencia). Build verificado.
- **v18.9.1** (17 jun) — **limpieza del row de actividades + menú de click derecho enriquecido** (feedback de Malio: "se ve saturado"). (1) **Grip de reorden solo en hover** (antes siempre visible → saturaba). (2) **Quitados del row**: peso ponderado y horas estimadas (nadie los usaba en la tabla; horas sigue editable en el panel de la actividad), y los botones inline +sub / Eliminar (ahora en el menú). (3) **"i" de Info aparece al pasar el cursor**, al final de la fila. (4) **Menú de click derecho** (sobre actividad, en tabla y Gantt) al estilo Monday: Abrir actividad · Abrir en una pestaña nueva · Crear actividad nueva abajo (crea hermano y lo coloca justo debajo reusando la lógica de reorden) · Agregar sub-actividad · Duplicar · Importancia · Hito · Eliminar. Quitado "Copiar enlace". **Pendiente (riesgo, sesión dedicada)**: "Mover a" y "Convertir a sub-actividad" (re-parentar arrastrando) y "Archivar" (no existe ese estado hoy). Build verificado.
- **v18.9.0** (17 jun) — **reordenar actividades por drag & drop en la tabla**. A la izquierda de cada actividad/sub-actividad aparece un handle de 6 puntos (⠿); se arrastra y al soltar sobre otra fila toma su posición y el resto se recorre (ej.: mover la actividad 4 arriba de la 2 → pasa a ser 2). La numeración (1, 1.1, 2…) es **posicional** así que se recalcula sola. Alcance: **entre hermanos del mismo nivel** (mismo `parent_id`); al arrastrar un padre su subárbol viaja con él. Como `numero` es UNIQUE por proyecto, el reorden reasigna el grupo a un bloque por encima del máximo global (cero colisiones, incluso con updates en paralelo) vía helper `reordenarActividades`. Solo desktop (el drag en móvil queda pendiente). **Pendiente para sesión dedicada**: reordenar arrastrando *dentro* del Gantt (su drag de fechas/dependencias es delicado — el nuevo orden igual se refleja ahí porque comparte el orden por `numero`) y re-parentar arrastrando. Build verificado; QA en navegador pendiente de Malio.

- **v18.8.2** (17 jun) — fix del administrador de Horas: las actividades tienen `horas_estimadas = 0` por default (no NULL) → "0" es "sin estimar" (7,195 de 7,235 actividades). El botón "default global" filtraba solo `IS NULL` → no hacía nada; ahora alcanza `NULL` o `0` (verificado: 7,195 filas). El resumen muestra `0h` como "—" (sin horas) para que se vea qué falta configurar. Tab Horas verificado en navegador: abre sin errores, lista 641 tipos de actividad. (Fix de v18.8.1 confirmado: cero variables indefinidas en todo `src/`.)
- **v18.8.1** (17 jun) — **FIX pantalla blanca al abrir Configuración → Horas** + red de seguridad global. El tab Horas usaba `inputStyle` sin importarlo → ReferenceError → app en blanco (Vite no detecta variables indefinidas en JS puro, por eso el build pasaba). Agregado el import. **+ ErrorBoundary global** en `App.jsx`: de ahora en más, un error de runtime en cualquier módulo muestra una pantalla de error con el mensaje + botones "Ir al Dashboard" / "Recargar", en vez de dejar toda la app en blanco. El resto de la app sigue usable.
- **v18.8.0** (12 jun) — **gestión masiva de horas estimadas**. (1) Guardado con confirmación visible: al setear horas en el panel de actividad aparece toast "✓ Nh guardadas". (2) **"Aplicar a todas"**: botón en el panel que aplica esas horas a TODAS las actividades homónimas en todos los proyectos (las actividades vienen de plantillas → el mismo nombre se repite; reporta cuántas actualizó). (3) **Configuración → tab "Horas"** (roles de gestión de proyectos): administrador masivo — lista de tipos de actividad (agrupados por nombre, con repeticiones y horas actuales), búsqueda, asignar horas por tipo, y **default global** para todas las que no tienen horas (sin pisar las configuradas). Helpers nuevos: `aplicarHorasPorNombre`, `getResumenHorasActividades`, `asignarHorasFaltantes`.
- **v18.7.2** (12 jun) — **fixes del feedback de Mayra (vista Personas)**. (1) **Horas estimadas editables desde el panel de la actividad**: el input solo existía en la tabla de actividades — al abrir una actividad específica no había forma de ajustar el esfuerzo, y el cálculo de carga asumía 8h/día → sobrecargas absurdas (905%, desviación +14729%). Ahora el panel tiene campo "Horas estimadas (esfuerzo)" junto a la duración; al setearlo, la carga en Personas se prorratea con horas reales. (2) **"Abrir →" llega siempre a la actividad**: los deep-links de Proyectos (`?proyecto=&actividad=`) eran one-shot al montar — el segundo clic no abría nada; ahora leen la URL en vivo (mismo patrón del fix de Ventas) y el panel se re-abre incluso para otra actividad del mismo proyecto. Navegación desde Plantas (location.state) preservada en efecto propio.
- **v18.7.1** (12 jun) — **AUDITORÍA ANTI-ERRORES** (el equipo reportaba errores que frenaban el trabajo). Evidencia: logs de producción + emulación de los 12 caminos de escritura por rol (todos OK) + revisión adversarial del código v18. **4 bugs encontrados y corregidos**: (1) **CRÍTICO — "no me deja crear"**: `crearLead`/`crearCotizacion` generaban el código con `count()` filtrado por RLS → colisiones de UNIQUE intermitentes (la queja de Regino de hace semanas); ahora la BD asigna el código vía secuencia+trigger (`LEAD-1000+`/`COT-1000+`), inmune a RLS y concurrencia — verificado por emulación. (2) **ALTO — el textarea de notas en Ventas perdía el foco en cada tecla** (componente `Seccion` definido dentro del panel → remontaje en cada render); movido a nivel módulo. (3) Dashboard: widget "Cotizaciones pendientes" y serie "Cotizado" mensual usaban `c.monto_total` (campo inexistente → siempre $0); ahora `c.total`. (4) Deep-links de Ventas ahora se re-aplican aunque ya estés en el módulo. Confirmado por logs: queries nuevas (montos/realtime) responden 200 en producción. Pendiente menor: 2× POST 400 esporádicos en `usuario_preferencias` (fire-and-forget, no afecta UX).
- **v18.7.0** (12 jun) — **Lockdown de dinero TOTAL — auditoría exhaustiva + guardas anti-regresión**. Fugas cerradas: (1) `notificaciones` — la cláusula `destinatario_id IS NULL` dejaba leer broadcasts de otros roles (el workflow escribe montos en el texto); ahora solo propias o del rol propio. (2) `precios_servicios` — catálogo de precios era legible por cualquier autenticado; ahora solo direccion/admin/ventas. (3) `actividades.monto_cobrable` — columna de dinero en tabla legible por proyectos, nunca usada (0 filas) → **eliminada** (input del editor removido, `marcarCobrable` sin monto, `duplicar_proyecto` actualizado); el dinero de cobro vive solo en `hitos_cobranza`. **Auditoría final: ambos roles ven 0 filas en las 12 superficies de dinero** (emulación RLS con usuarios reales) y sus 198 proyectos intactos. **Guardas permanentes**: `scripts/auditoria_lockdown_dinero.sql` (correr tras cualquier cambio de permisos; veredicto automático OK/FUGA) + regla innegociable documentada en CLAUDE.md.
- **v18.6.0** (12 jun) — **(1) Blindaje BD de monto_contrato**: la columna se movió a tabla `proyectos_montos` con RLS exclusiva de roles de dinero (read: direccion/admin/ventas/cobranza; write: direccion/admin) — los roles de proyectos no pueden leer el monto **ni por API**. Columna `proyectos.monto_contrato` borrada; el trigger del workflow escribe en la tabla nueva; `getProyectos` la embebe (`montos:proyectos_montos`) y Cierre lee `p.montos?.monto_contrato`. Sin pérdida de datos (los 198 proyectos tenían monto NULL — histórico Monday). **(2) Realtime en Ventas** (frente captura sin fricción): `leads` y `cotizaciones` agregadas a la publicación realtime; el pipeline se actualiza en vivo cuando otro usuario cambia algo (recarga silenciosa con debounce, respeta RLS). Frentes que quedan requieren cuentas/decisiones externas: integración email/WhatsApp; modo simple del detalle de proyecto (requiere definición de producto).
- **v18.5.0** (12 jun) — **(1) Lockdown de dinero para roles de proyectos** (pedido de dirección): `director_proyectos` y `equipo_proyectos` sin acceso a NADA financiero. BD: `hitos_cobranza` ya no incluye director_proyectos (única tabla de dinero que lo tenía; contratos/cotizaciones/facturas/compras/cxp/gastos ya lo excluían). Frontend: director_proyectos pierde el módulo `ventas`; Dashboard no fetcha datos financieros para ambos roles (`skipFinanciero`) y la vista Ejecutivo oculta KPIs de dinero/Pipeline comercial/Cobranza vencida/Cotizaciones pendientes (en su lugar: KPI "Cierres próximos" + acción "Ver proyectos"); Cierre oculta la columna Monto. **(2) Alerta "próxima acción vencida"**: integrada a `leads_sin_actividad` (severidad importante, detalle con la acción y fecha; deep-link al pipeline). **(3) Captura rápida**: `/ventas?nuevo=1` abre el alta directo; la acción rápida del Dashboard lo usa.
- **v18.4.0** (12 jun) — **(1) Formato tabla estándar en TODA la app**: Cobranza (hitos + por cliente), Facturación, Compras, Contratos, Cierre y Postventa convertidos al mismo formato de tabla de Ventas/Proyectos (header uppercase, filas grid con hover, primera columna principal + sub-línea, montos mono, badges). Auditado en navegador: los 6 módulos cargan sin errores (Cierre verificado con datos reales). **(2) Edición plena para equipo_proyectos** (pedido de dirección): cualquier usuario del área de proyectos edita proyectos y mueve TODAS las actividades — se quitó el requisito de membresía (`app_es_miembro_proyecto`) que causaba fricción. RLS (`v18.4.0_equipo_proyectos_edicion_plena.sql`: acts_insert/update/delete + proy_update) + frontend (`puedeGestionarProyecto` incluye equipo_proyectos; `puedeEditarAct` del Gantt ya no restringe por responsable). Verificado vía emulación RLS con usuario real (UPDATE proyecto y actividad ajena = 1 fila, antes 0). Crear proyectos sigue en gestión; borrar solo dirección.
- **v18.3.0** (11 jun) — **lista de Proyectos en formato tabla** (mismo formato que la Tabla de Ventas — un solo formato en toda la app, pedido de Malio): encabezados Proyecto/Estado/Tipo/Avance/Cierre, filas alineadas con hover, código+chips+cliente·director en la primera columna, avance mono coloreado, fecha de cierre. Mobile: 2 columnas (proyecto+avance, badge de estado inline). Verificado visualmente en navegador.
- **v18.2.0** (11 jun) — **cierre de huecos vs investigación CRM**. (1) **Próxima acción con fecha** en cada oportunidad (insight #1 de Pipedrive: accountability de follow-up): campos `leads.proxima_accion`/`proxima_accion_fecha` (migración), edición en el panel, **chip visible en la tarjeta del Kanban** (rojo = vencida, amber = hoy). (2) **Dashboard coherente con Ventas**: widget "Pipeline comercial" agrupa por las 5 fases (antes 7 etapas viejas), acciones rápidas → "Nueva oportunidad" (/ventas). (3) **Drill-downs arreglados**: Ventas lee `?filtro=pendientes|sin_respuesta` del Dashboard (con chip para quitar el filtro); antes el filtro se perdía en el redirect.
- **v18.1.0** (11 jun) — refinamientos post-QA del módulo Ventas + pulido Proyectos. (1) **Orden** en Ventas: `SortControl` (default más recientes primero, persistido), ordena Kanban y Tabla. (2) **Vista previa de partidas**: el detalle de oportunidad muestra qué se cotizó (partidas de la cotización) sin abrir la cotización completa. (3) **Proyectos**: tarjetas pulidas al estándar de Ventas (hover shadow, radio 12, tipografía). QA del módulo Ventas hecho en navegador (Playwright, usuario temporal) — Kanban/detalle/alta/generar cotización/abrir cotización verificados; cero errores de runtime.
...
- **v17.8.0** (10 jun) — **hardening de seguridad (auditoría)**: (P0.2) `search_path` fijo en `cotizacion_aprobar_workflow` (única SECURITY DEFINER sin él). (P1.1) políticas de `leads`/`cotizaciones` de `{public}`→`authenticated` — NO eran explotables por anon (sus `USING` usan `app_has_any_rol()`/`app_user_id()` que dan false/null para anon) pero unifica el modelo. (P1.2) guardas de autorización dentro de `duplicar_proyecto`/`duplicar_actividad` (eran SECURITY DEFINER que bypassan RLS → cualquier autenticado podía duplicar ajenos; ahora exigen rol de gestión/membresía). (P1.5) edge function `alerta-acceso-cotizaciones`: email destino/origen configurables vía secret `ALERT_EMAIL`/`ALERT_FROM_EMAIL` con fallback. `.gitignore`: ignora backups locales + basura de templates. Plan completo de auditoría en `PLAN_TRABAJO.md`. **NO revocado a propósito**: EXECUTE en helpers RLS (`app_*`) — son llamados dentro de las políticas, revocarlos rompería toda query con RLS.
- **v17.7.0** (10 jun) — **filtro por año en Leads + Dashboard (dinámico)**. (1) Leads: mismos filtros que Cotizaciones (búsqueda + año + mes), default = **año actual** vía `ANIO_ACTUAL`/`aniosDisponibles()` en `helpers.jsx` (las opciones y el default se recalculan solos cada año — cuando llegue 2027 aparece y queda por defecto). (2) Dashboard: filtro por año (cotizaciones por `fecha_emision`, leads por `created_at`), default = año actual, selector en el header (oculto para equipo_proyectos). (3) Cotizaciones: opciones de año ahora dinámicas (antes hardcoded 2024/2025/2026). (4) Backfill: se completaron leads espejo para **todas** las cotizaciones (527 leads: 247 de 2024, 185 de 2025, 95 de 2026) y se fijó `lead.created_at = fecha_emision` de su cotización; el trigger forward (`v17.7.0_lead_created_at_from_cotizacion.sql`) hereda esa fecha en altas nuevas.
